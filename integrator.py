from nbody import Body, Active_Body, NBody
...
class RK_4Integrator:
    ...
    def get_new_positions(self):
        ...
        for body in self.n_body.n_body:
            ...
            body.reset_force()
            if isinstance(body, Active_Body) is True \
                    and self.t < 0.00223 and body.boolean is True:
                body.propulsion = 1.623e-15
            elif isinstance(body, Active_Body) is True and self.t >\
                    0.00223 and self.t < 0.0071 and body.boolean is True:
                body.propulsion = 2.453e-16
                body.m = 8.8e-17  # 3.06687e-16 - 2.18e-16
            elif isinstance(body, Active_Body) is True and self.t >\
                    0.0071 and self.t < 0.00927 and body.boolean is True:
                body.propulsion = 4.8e-17
                body.m = 3.99e-17  # 8.8e-17 - 4.81e-17
            elif isinstance(body, Active_Body) is True and body.boolean:
                body.angle += body.increment
            elif isinstance(body, Active_Body) is True and self.t > 0.00927\
                    and body.boolean is True:
                body.propulsion = 0
...
class RK_4_ajuste:
    ...
    # sorte de version intermédiaire entre RK4_Integrator et le final
    ...
    def get_new_positions(self):
        ...
        k2 = [self.dt / 2 * (v + self.dt / 2 * k)
              for v, k in zip([body.v for body in temp_n_body_k2.n_body], k1)]
        k2_v = [self.dt / 2 * (a + self.dt / 2 * k1)
                for a, k1 in zip(a_i2, k1_v)]
        ...
        k3 = [self.dt / 2 * (v + self.dt / 2 * k)
              for v, k in zip([body.v for body in temp_n_body_k3.n_body], k2)]
        k3_v = [self.dt / 2 * (a + self.dt / 2 * k2)
                for a, k2 in zip(a_i3, k2_v)]
        ...
        k4 = [self.dt * (v + self.dt / 2 * k)
              for v, k in zip([body.v for body in temp_n_body_k4.n_body], k3)]
        k4_v = [self.dt * (a + self.dt / 2 * k3) for a, k3 in zip(a_i4, k3_v)]

        for i, body in enumerate(self.n_body.n_body):
            r_i = body.r + self.dt / 6 * (k1[i] + 2 *
                                          k2[i] + 2 * k3[i] + k4[i])
            body.r = r_i
            new_positions.append(r_i)

        for i, body in enumerate(self.n_body.n_body):
            v_i = body.v + self.dt / 6 * (k1_v[i] + 2 * k2_v[i] + 2 *
                                          k3_v[i] + k4_v[i])
            body.v = v_i
            new_vitesses.append(v_i)
            body.reset_force()
...
class RK4_killian:
    ...
    def get_new_positions(self):
        ...
        for i in range(l):
            ...
            pos3[i] = BINT1.n_body.n_body[i].r + (pow(self.dt, 2)/4) * k1[i]
            # self.n_body.n_body[i].vitesse + (pow(self.dt,2)/4) * k1[i]
            bodyk3.append(Body(BINT1.n_body.n_body[i].m, pos3[i].x,
                               pos3[i].y, 0, 0))
        ...
        for i in range(l):
            ...
            pos4[i] = self.n_body.n_body[i].r + self.dt * \
                self.n_body.n_body[i].v + (pow(self.dt, 2)/2)*k2[i]
            bodyk4[i] = Body(self.n_body.n_body[i].m, pos4[i].x,
                             pos4[i].y, 0, 0)
        ...
        for i in range(l):
            self.n_body.n_body[i].r += self.dt*self.n_body.n_body[i].v +\
                                       (pow(self.dt, 2)/6) *\
                                       (k1[i] + k2[i]+k3[i])
            self.n_body.n_body[i].v += (self.dt/6) *\
...
# créer une nouvelle liste de Body mais avec
        # les variations de vitesse et de position
        nbody2 = NBody([
            Body(body.m, body.r.x + k1x[i].x/2, body.r.y + k1x[i].y/2,
                 body.v.x + k1v[i].x/2, body.v.y + k1v[1].y/2)
            for i, body in enumerate(self.n_body.n_body)
                        ])
        ...
        nbody3 = NBody([Body(body.m, body.r.x + k2x[i].x / 2, body.r.y +
                             k2x[i].y / 2, body.v.x + k2v[i].x / 2,
                             body.v.y + k2v[1].y / 2)
                        for i, body in enumerate(self.n_body.n_body)
                        ])
        ...
        k3x = [(body.v + k2v[i] / 2) * self.dt
               for i, body in enumerate(nbody3)]

        nbody4 = NBody([Body(body.m, body.r.x + k3x[i].y, body.r.y + k3x[i].y,
                             body.v.x + k3v[i].x, body.v.y + k3v[1].y)
                       for i, body in enumerate(self.n_body.n_body)
                        ])
        ...
        for i, body in enumerate(self.n_body.n_body):
            ...
            """if self.t > 0.217 and self.t < 0.218:
                print("pos lune", self.n_body.n_body[1].r.x,
                self.n_body.n_body[1].r.y,"pos apollo",
                self.n_body.n_body[2].r.x,
                self.n_body.n_body[2].r.y)"""
...
class RK_final2:
    ...
    def RK_new_step(self, factor, kx_avant, kv_avant):
        n_body = NBody([
            Body(body.m, body.r.x + kx_avant[i].x * factor,
                 body.r.y + kx_avant[i].y * factor, body.v.x + kv_avant[i].x
                 * factor, body.v.y + kv_avant[i].y * factor)
            for i, body in enumerate(self.n_body.n_body)
        ])
        n_body.compute_resultante()
        a = [body.res_force / body.m for body in n_body]
        kv = [a[i] * self.dt for i in range(self.len_n)]
        kx = [(body.v + kv_avant[i] * factor) * self.dt
              for i, body in enumerate(n_body)]

        return kx, kv

    def get_new_positions(self):
        print((self.n_body.n_body[1].r-self.n_body.n_body[0].r).norm())
        k1x, k1v = self.RK_new_step(1, [Vector(0, 0)]*self.len_n,
                                    [Vector(0, 0)]*self.len_n)
    ...
    def get_new_positions(self):
        ...
        if self.t == 0:
            ...
            for i, body in enumerate(self.n_body):
                new_position = 2 * body.r - self.previous_positions[i]\
                               + body.res_force / body.m * self.dt ** 2
                body.v = (new_position - self.previous_positions[i])\
                    / (2 * self.dt)
