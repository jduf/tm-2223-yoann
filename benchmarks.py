...
from RungeKutta_Killian import RK4
...
"""n_body = NBody(
        [
            ...
            Active_Body(0.2887e-17, 2.704, 2.945, 9.29595, -16.117, 0.002678, 10, False)
...
for t in range(218, 278):
    t = round(t * 0.001, 3)  # Convertit le pas de temps en secondes
    x_positions.append(cos(radians(360*t / 2.09)) * 4 + 0.019 * 0.019 * cos(radians(127.3779 - 360 * (t - 0.218) / 0.005714)))
    y_positions.append(sin(radians(360*t / 2.09)) * 4 + 0.019 * sin(radians(127.3779 - 360 * (t - 0.218) / 0.005714)))
...
for t in [0.0, 0.217, 0.277]:
    plt.scatter(x_positions[int(t*1000)], y_positions[int(t*1000)], label=f'T = {t}', s=50)
